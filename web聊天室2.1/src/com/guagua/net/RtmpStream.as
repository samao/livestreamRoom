package com.guagua.net 
{
	...
	public class RtmpStream extends EventDispatcher 
	{
		...
		{
			//音频状态
			if (videoStream && videoStream.info)
			{
				execute(new RtmpEvent(RtmpEvent.RTMP_AUDIO_CHANGE, videoStream.info.audioBytesPerSecond > 0));			
			}			
		...
		private function volumeHandler(e:RtmpEvent):void 
		{
			...
			if (videoStream)
			{
				videoStream.soundTransform = volume;	
			}					
		}
		...
		private function netStatusHandler(e:NetStatusEvent):void 
		{
			...
			switch(e.info.code)
			{
				...
				case "NetStream.Play.Start":
					...
					break;
				default:
						Log.out(micIndex,"麦：rtmp 没有处理消息：", e.info.code,e.info.description);
					break;					
			}
		}
